...
| **Fuel is deterministic even when output is not — non-branching case only** | **B** | S57. `test_gnd_conv.metta` calls `(flip)`: three different result hashes across three platforms, `fuel_used = 1012` on all three. **The meter is separable from the result.** Held 30/30 runs, 18 distinct outputs. **But nothing in that program branches on the random value**, so control flow is fixed and fuel *cannot* vary — this is the trivial case. `(if (flip) (long) 0)` is untested and the billing design needs it |
