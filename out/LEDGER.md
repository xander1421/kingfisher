...
| **Checkpoint commitments now REPRODUCE** | **B** | **S65**, and this is what the hyperon patches bought: 6 corpus programs, every run one Merkle root. S60 could not achieve this — its digests varied because `Display` leaked heap addresses. Checkpoint hashing, bisection and dispute proofs were all genuinely blocked behind that fix |
| **Checkpoint retention is hundreds of bytes, not megabytes** | **B** | S65: 22 checkpoints in 50,794 steps → **704 B retained, 21 inner hashes, 5 bisection probes**. Corpus range 192 B–1,088 B, 3–6 probes. S60's "32N bytes = 1.6 MB" assumed a checkpoint **per step**; they fire on observable state change only |
| **A dispute resolves to an interval of 1,025–6,298 steps** | **B** | S65. So the risc0 figure to measure is **one-interval proving**, not one-step. Interval is tunable downward at ~32 B and one hash per extra checkpoint — commitment is nearly free, so the trade favours more checkpoints |
...
| **MeTTa's reduction path is already zkVM-clean** | **B** | **S66.** `hyperon-atom`/`-common`/`-space` and `interpreter.rs` use **zero** filesystem, threads, clock or network. All 75 fs sites are in the runner (module loading, catalog, `fileio`), which a dispute proof does not need — bisection starts from a committed state, so the guest never loads a module. The port is bounded to the runner boundary |
| hyperon uses **no threads at all** | **B** | S66. A11's threaded-accumulation concern applies to DAS's attention broker, not this engine |
