...
| **A job that pins its generator is byte-identical AND fuel-identical across two ISAs** | **B** | S58: `(new-random-generator 42)` → fuel 1948, hash `16b0e4996b`, **22 runs across aarch64-macOS, x86_64-macOS, aarch64-Android**. Control `b5` differs by one token and varies on all platforms |
| **`flip` and `&rng` must be banned from the job class** | **B** | `random.rs:186-188` — `flip` takes no generator and calls global `rand::random()`; `&rng` is `from_os_rng()` bound at module load (`:127-128`). Neither is reachable by `set-random-seed`. **Statically checkable before scheduling — the cheapest enforcement point we have** |
