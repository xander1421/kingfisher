...
| **DAS's `double` ECAN fails the N-thread==1-thread oracle; fixed-point to the A11 spec passes** | **A** | **S67**, reproduced 3×. 4,096 nodes, 12 epochs: `double` gives **four different state hashes for T=1/2/4/8**; fixed-point gives one. Anti-degeneracy guard asserts >half the nodes move and >100 distinct values. Reference implementation in `spikes/S67_ecan_fixedpoint/`. The cause is the **rate multiplication rounding plus non-associative float addition**, not integer addition — "use integers" is the wrong summary of the fix |
| **DAS attention broker is nondeterministic by construction** | **B** | `typedef double ImportanceType` (`HebbianNetwork.h:17`), and `total_rent += rent` accumulates over a trie walk with `WorkerThreads.cc` present. **Float summation across threads is order-dependent.** Not the S59 transcendental problem — pure accumulation order. Fix: fixed-point importance; stimulus is already `unsigned int` |
