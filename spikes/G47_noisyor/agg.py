...
# -----------------------------------------------------------------------------
# ---- G47: THE ONE VARIABLE -------------------------------------------------
# `AGG` selects how the confidences of several rules firing for the SAME
# candidate are combined. Everything else in this file is byte-identical to
# `spikes/G36_repro_g34/length1_constants.py` -- the copy is deliberate (a
# mid-sweep edit to a shared generator is the `pick_parent` contamination C7
# paid for) and both digests are recorded in `agg.json`.
#
#   "max"      what G34 published: score = max(conf_i). AnyBURL's max-plus.
#   "noisy_or" score = 1 - prod(1 - conf_i), which rewards a candidate that
#              several independent rules agree on. G30's own analysis named
#              this as a difference between this miner and AnyBURL/AMIE+.
#
# There is no third arm and no tuned constant: A26 says a knob is not a
# mechanism, and noisy-OR has no free parameter to fit.
AGG = os.environ.get("G47_AGG", "max")


def bump(d, k, conf):
    """Combine `conf` into candidate `k`'s running score under AGG."""
    if AGG == "max":
        if conf > d[k]:
            d[k] = conf
    else:                       # noisy_or
        d[k] = 1.0 - (1.0 - d[k]) * (1.0 - conf)
    ...
    for p, s, o in test_triples:
                    ...
                    if pp1 == p1 and b_node != s:
                        for pp2, c_node in out_adj.get(b_node, ()):
                            if pp2 == p2 and c_node != s and c_node != b_node:
                                bump(cand_scores_tail, c_node, conf)
        ...
        if rules_const_tail:
            ...
            for r in rules_const_tail.get(p, ()):
                ...
                if q in s_preds:
                    bump(cand_scores_tail, c_const, conf)
                    ...
                    if pp2 == p2 and b_node != o:
                        for pp1, a_node in in_adj.get(b_node, ()):
                            if pp1 == p1 and a_node != o and a_node != b_node:
                                bump(cand_scores_head, a_node, conf)
        ...
        if rules_const_head:
            ...
            for r in rules_const_head.get(p, ()):
                q, c_const, conf = r["body"], r["const"], r["conf"]
                if q in o_preds:
                    bump(cand_scores_head, c_const, conf)
