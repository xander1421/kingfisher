...
# path-limited `git log -1` (~60 ms) and a full-history `git log --grep` (~14 ms)
# for each of 344 files, 688 invocations -- so its cost was O(files x history) in
# a repo where both grow every cycle. It finished in ~20 s when published and
# exceeded 2 minutes one cycle later. CLASS: a diagnostic whose cost scales with
# the thing it measures dies exactly when it becomes useful (H14 by another road).
#
...
printf '%s\n' "$out" > "${DUMP:?KFPROBE dump path required}"   # KFPROBE
