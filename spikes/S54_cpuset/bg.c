...
static double run_cfg(int nt, long nrows) {
    ...
    static const int order[7] = {0,1,4,5,2,3,6};
    NT = nt; NROWS = nrows;
...
int main(void) {
    ...
    cpu_set_t set; CPU_ZERO(&set); CPU_SET(6,&set);   /* coordinator outside the background set */
    sched_setaffinity(0,sizeof set,&set);
    double mhz = measure_mhz();
    printf("S53 residency v2 — cycles/row, the governor-invariant unit\n");
    long sk = sysfs_khz(6);
    printf("BACKGROUND cpuset reality: workers restricted to cpu0,1,4,5\n");
    printf("coordinator cpu6, measured %.0f MHz\n", mhz);
