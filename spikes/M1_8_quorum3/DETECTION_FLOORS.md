...
| **corpus fault-expression** | a mutation the corpus can express | **measured, not assumed** (`M1_9_mutation`, 4 mutations): wrong `-` **4/64**, wrong `<` **0/64**, resolver text **24/64**, extra stdlib rule **0/64**. Classes partition by layer — the 24 import-failures are the ONLY detector of resolver faults, the 14 empties detect nothing. Two blind spots: `<` vs `<=` differ only at equality and no program prints that case; and `fuel_used` does not move when the stdlib grows, so altered stdlib rules are invisible unless invoked. Executing a faulty line is not observing its fault |
