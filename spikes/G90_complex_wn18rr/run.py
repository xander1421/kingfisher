...
Trains ComplEx (dim=64, 1-N softmax AdaGrad) on official WN18RR (86,835 train triples, 40,943 entities, 11 relations)
...
def train_complex_wn(train_triples, nent, npred, epochs=8, lr=0.10, bsz=1024, reg=1e-4, seed=42):
    print(f"Training ComplEx on WN18RR (nent={nent}, npred={npred}, dim={DIM}, epochs={epochs})...")
    ...
    for ep in range(1, epochs + 1):
        ...
        for start in range(0, n, bsz):
            ...
            sc_t, ire, iim, h_re, h_im, r_re, r_im = complex_tail_scores(E_re, E_im, R_re, R_im, s, p)
            ...
            # 2. Head Prediction: (?, p, o) -> s
            sc_h, rre, rim, t_re, t_im, r_re, r_im = complex_head_scores(E_re, E_im, R_re, R_im, o, p)
    ...
    print(f"ComplEx trained in {time.time()-t_train0:.2f}s. Loss: {epoch_losses[0]:.4f} -> {epoch_losses[-1]:.4f}")
    return E_re, E_im, R_re, R_im, epoch_losses


def evaluate_complex_wn(test_triples, E_re, E_im, R_re, R_im, true_sp, true_po):
    print(f"\nEvaluating ComplEx on {len(test_triples)} test triples (6,268 queries)...", flush=True)
    t0 = time.time()
    ...
    for start in range(0, n, eval_bsz):
        ...
        # Tail scores: (B, nent)
        sc_t, *_ = complex_tail_scores(E_re, E_im, R_re, R_im, s_b, p_b)
        ...
        # Head scores: (B, nent)
        sc_h, *_ = complex_head_scores(E_re, E_im, R_re, R_im, o_b, p_b)
        for i in range(len(batch)):
            s, p, o = int(s_b[i]), int(p_b[i]), int(o_b[i])
            scores = sc_h[i]
            tgt_score = scores[s]
    ...
    print(f"ComplEx Test Evaluation finished in {time.time()-t0:.2f}s ({n_q} queries):", flush=True)
...
def main() -> int:
    t0 = time.time()
    print("=== Spike G90: ComplEx Latent Embedding on Official WN18RR ===", flush=True)
    ...
    E_re, E_im, R_re, R_im, losses = train_complex_wn(
        train, nent, npred, epochs=EPOCHS, lr=LR, bsz=BATCH_SIZE, reg=REG, seed=SEED)

    metrics = evaluate_complex_wn(test, E_re, E_im, R_re, R_im, true_sp, true_po)
    ...
    falsifiers = [
        ...
        Falsifier("F2_fails_symbolic_baseline", refutes="that ComplEx outperforms pure 2-hop symbolic rules (0.0355 MRR) on WN18RR", fires_when="mrr <= 0.0355", null_must_contain="ComplEx <= 0.0355"),
    ...
    falsifiers[1].observe(f2, {"complex_mrr": metrics["mrr"], "symbolic_mrr": 0.0355})
    ...
    res = {
        ...
        "model": "ComplEx (dim=64, 1-N Softmax AdaGrad)",
        "training": {
            "epochs": EPOCHS,
            "lr": LR,
    ...
    ok, problems = kfcheck.certify(
        ...
        falsifier="ComplEx on WN18RR fails training or evaluation",
        allow_dirty=True,
        note="G90: ComplEx Latent Embedding Training & Evaluation on Official WN18RR.",
    )
    print(f"\nD6 Provenance Certified: ok={ok}", flush=True)
    for pr in problems:
        print(f"  PROBLEM: {pr}", flush=True)

    print(f"\n=== Spike G90 Completed in {time.time()-t0:.2f}s ===", flush=True)
